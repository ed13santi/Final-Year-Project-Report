\chapter{Design and testing of the proposed decomposition technique}
...
\subsection[Obtaining the state trans. matrix]{Obtaining the state transition matrix}
...
to drop-off the passenger successfully. Due to the limited computing power of my tools, this was a probelm as it did not allow for
significant testing with a high number of trials. Thus it was decided to use a simpler environment for easier experimentation and
one that would also allow for easier interpretation of the results. I thus introduced the 'Grid' environment, which is a simple 25
by 25 grid with two destinations, as shown in \ref{fig:grid}. At the start of the episode, the Taxi is initialised in a random position, while the passenger
is always assumed to be inside the Taxi. The pick-up and drop-off are eliminated, so that the learning process only deals with
learning how to navigate in the grid. The two destinations are located at opposite corners. throughout an episode the destination
is always the same as in the initial state. To summarise, the environment has:
\begin{itemize}
...
For a complete evaluation of the state-decomposition method, it needs to be tested in a more generic environemnt, with non-zero
...
of changing the destination whenever the current destination is the one closest to the current position in the grid. This modified
'Grid' environment will be referred to as 'GridEps'. The following table summarises the transition probabilities in 'GridEps'.
...
\begin{table}[H]
    ...
    \multicolumn{1}{|c|}{\multirow{4}{*}{state}} & triangle 1 - destination 1 & 1-\textbackslash{}epsilon          & epsilon                   \\
    \multicolumn{1}{|c|}{}                       & triangle 1 - destination 2 & 0                                  & 1                         \\
    \multicolumn{1}{|c|}{}                       & triangle 2 - destination 1 & 1                                  & 0                         \\
    \multicolumn{1}{|c|}{}                       & triangle 2 - destination 2 & \textbackslash{}epsilon            & 1-\textbackslash{}epsilon \\ \hline
    \end{tabular}
    
    \caption{State transition probabilities in the 'GridEps' environment.}
\end{table}

The reason that the transitions were implmented in this way, rather than ISTHAT IM DUMBO, THERE IS NO REASON FOR THIS, JUST ME RUSHING THINGS CUZ I HAD TO DO EXAMS
...
providing a fair comparison can be a non-trivial task, as discussed in the following subsection. 
...
corresponding outputs and $f_{\theta}$ is the function implemented by the neural network with weights $\theta$. The network
updates are done using the optimiser Adam \cite{kingma_adam_2017}, which was shown to have better performance than simple gradient
descent, through the use of weight momentums.

Each iteration with the environment is saved in a replay memory. The maximum size of this is set to be big enough so that all
...
one update step of the neural network using a batch of 128 samples, randomly picked from the replay memory.

COULD ADD IMAGE OF NETWORK

\section{Ensuring fair comparison}
...
Unfortunately, it is not possible to do so due to the complexity of the relationship between various hyperparameters. However, it
is possible to make reasonable choices such the comparison is as fair as possible. In this project, it was aimed to keep all
hyperparameters constant in all trials. Different experiments that involve different network architectures, such as using 2 neural
networks instead of 1 always ensure that the total number of weights in each agent is approximately equal. Other factors such the
encoding method of the state are always kept equal, unless the objective of the experiment is to investigate their effect on the
performance.
