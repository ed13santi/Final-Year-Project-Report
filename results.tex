...
EXPLAIN HOW EXPERIEMNTS WERE RUN, NUMBER OF OF TRIALS, USE OF CONFIDENCE BOUNDS, USE OF SAMPLE GRAPHS and also probelm with sample
graphs so why have to use shorter versions

\section{Effect of different state representations on the performance}
...
state to be fed into the neural network. As a first approach, the state was fed directly as an integer between 0 and $no. states -
1$. This is the most compact encoding of the state which is what one would use in standard Q-learning, in which the state is used
merely as an index in a look-up table rather than as the input of a neural network. Though extremely compact, this representation
of the state is very hard to decode for the neural network and its structure does not directly reveal aspects of the speed
directly\footnote{As the state is represented by an integer, this has not physical meaning before decoding. However, the state
could be represented for example by a vector of values, each having a physical meaning such as horizontal position, etc.} and led
to difficulties in achieving convergence in a reasonable number of iterations. Thus it was decided the encode the state in in
three alternative representations:

\begin{enumerate}
    \item \textbf{Compact} The idea is to give a physical meaning to the state. The state is represented by a vector of values,
    each representing a different aspect of the physical meaning of the state. In the 'Grid' environemnt, these are the row in
    which the Taxi is currently located, the column and its current destination. (NEED TO SAY FOR SPEICIFC ENV)
    \item \textbf{One-hot encoding} The idea is to make it easier for the neural network to learn to decode the state. The state
    is in this method simply converted to a one-hot encoded version.
    \item \textbf{Hybrid encoding} This combines the previous two methods. The destination is a represented by a single bit, while
    the position of the state is represented by a one-hot-encoded vector. This results in a state representation that is a vector
    of size one bit greater than the number of possible positions in the environment.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/effect_state_encoding.pdf}
    \caption{Effect of different state encoding methods on the performance of DeltaSwitch. IN WCHIH ENVIRONMENT??? SHOULD SHOW FOR DQN INSTEAD}
    \label{fig:effect-state-encoding}
\end{figure}

Figure-\ref{fig:effect-state-encoding} shows that in this environment (WHICH ENVIRONMENT?) the hybrid encoding achieves the best
performance, followed by the standard one-hot encoding and lastly by the compacy encoding. Thus, it was decided to use the hybrid
encoding for all experiments in order to expedite experimentation time. This was actually a majot issue during the project, as the
training time of the algorithms is long and any increase in performance is wellcome as it allows to tun more trials to obtain
higher quality results and to also run different experiments.

\section{First approach}
Need to describe initial results using the original method, which didn't allow communication between different sub-networks and
was a 2-stage method. Also the environment was too complicated and it was not easy to identify problems.

Don't even know if I should talk about these old methods because I don't even know what I achieved with them

The first meaningful results that I obtained used the original state-decomposition algorithm applied in a modified 'TaxiTraps'
environment. The changes made the environment simpler. The number of possible destinations was reduced to 2, while the passenger
is initialised to be always inside the Taxi, and the actions of picking-up and dropping-off were removed, so that the learning
task only involves learning to navigate. The probability of activating the Traps was set to 1\%. In this case, though the learning
was 


- Original TaxiTraps was problematic because the rewards used to get stuck at -200 (this is because of multi-sub-task nature of
the task) - Even with OneHot encoding it was so complicated that rewards would not improve, also extremely slow training, not good
for my purposes


\section{Initial results with Taxi2 and original state decomposition algorithm}

\section{Effect of varying the neural network's size}

It is important to note that the one of the factors influencing the speed of learning of deep reinforcement learning algorithm is
the size of the neural network. Figure-\ref{fig:effect_NN_dimension} shows that the performance seems to improve as the network
size increases, at least for the range of dimensions that I experimented with. When the network is too small, performance degrades
as the network is not complex enough to represent the function that maps from states to action-values of the environment, thus
rewards are lower after any number of episodes compared to other networks. In the figure, the three bigger networks are all
complex enough to reach convergence, whoever the biggest one does so quicker. This might be due to increased flexibility and the
existance of mutliple sets of weights that achieve the same optimal function representation. While this leads to faster
convergence and is not a problem in the environments used in this report, it might lead to problems in complex environments that
rely on generalisation of unseen states. This is because using an overly big network can easily cause overfitting, thus hurting
the generalisation propertties to unseesn states. Thus, for the sake of this project, it was decided to use networks that are
complex enough to capture the dynamics of the environment, while limiting the size so that the project could be relevant to other
scenarios. The figure also shows that the difference in performance of 'medium2', which has twice the number of trainable weights
of 'medium' is not very significant. I chose to show this comparison because in the environments used in this project, the
decomposition divides the state-space into two sub-spaces, which means that there are two NNs corresponding to the sub-spaces
instead of the one original NN. In order to make the comparison fair, I make the total number of trainable parameters
approximately equal in experiments that are being compared. It is arguable whether this is a fair comparison, but the fact that
the performance hardly varies when doubling the number of trainable parameters demonstrates that this is hardly an issue.
...
Though the original idea involves the use of a combinining neural network on top of the neural networks correspodning to each one
of the sub-spaces, I decided to experiment with a simpler approach in order to first validate the idea of state-decomposition in
the simplest possible application. For this purpose I introduced a network architecture that I called 'DeltaSwitch'. This consists
of one neural network corresponding to each sub-space, as in the orginal proposed architecture. The difference is that the
action-values are combined using a simple weighed sum, that is the output action-values of the combined architecture are given by
\begin{equation}
...
fact that it only consists of stage of training. In fact, all the networks are combined from the start and this is done in a fixed
way, as $\delta$ is a non-learned constant. The combining by weighed average effectively substitues the combining neural network
and acts as a switch when $\delta=0$, as it effectively selects the output of the network corresponding to sub-space of the
current space, thus the name 'DeltaSwitch'. If the special architecture of 'DeltaSwitch' is ignored, the algorithm is identical to
the basic DQN algorithm used as a benchmark, as the state decomposition acts directly on the 'switch'. $\delta$ is chosen to be
small, meaning that each sample used in an update of the network causes significant change in the weights of the corresponding
network, while only slightly changing those of other networks. This can be confirmed by deriving the formulas of the updated of
weights given the used loss function and optimiser, however it is sufficient to understand that those weights that have a smaller
effect on the output are updated by a smaller amount in the backpropagation update. As the network is always treated as a whole,
the target values required in the update of the network are allowed to use all the possible networks. This means that each network
is trained using target values generated by other networks, thus the networks are not independent during the training stage. This
is important as it was previously observed that not allowing this exchange of information during training leads to instability and
lack of convergence (WHERE WAS THIS OSBERVED).
...
The idea of this approach is that it might not be necessary to combine the action-values of the networks in a complicated manner
and that doing os using another neurla network might be complicated and lead to stability issues and lack of convergence.
...
performance between DQN and DeltaSwitch with differing $\delta$ values, as showing in Figure-\ref{fig:Taxi2-dqn-deltaSwitch}.
...
results. For this reason it was deemed important to repeat the experiment in an environment easier to interpret. This experiment
...
DeltaSwitch using $\delta=0$ presents some performance gain compared to DQN. This gain is epsecially noticeable when the state is
decomposed by position, meaning that one sub-space is formed by all those states in which the current position in the upper-left
triangle of the Grid, while the other sub-space is formed by those positions in the lower-right triangle. Decomposing by
destination means that the possible states are divided in two sub-spaces, corresponding to the two destinations.
...
'GridEps' environment with $\epsilon>0$. Figure-(WHATFIGURE!!!!!) compares the performance of training a DQN agent and 'DeltaSwitch'
agents, decomposing by destination and by position. It can be seen that for for $\epsilon \leq 0.3$ 'DeltaSwitch' with decomposition
by destination learns noticeably faster than the other two algorithms. The difference is especially large for $\epsilon = 0.3$. This
might be due to this environemnt being the harder to solve, giving a greater margin of improvement over DQN. For higher $\epsilon$
values, there isn't a statistically relevant difference between the three algorithms. This is probably due to the fact that the
environment is not as decomposable anymore, as the transition proabilities between the sub-spaces are larger.
...
As I used the 'hybrid' encoding described in a previous subsection, the input to the first layer of the network is a vectors of '0's
and '1's of size the number of possible positions in the grid plus one. However, since I am using $\delta=0$ and each network only
contributes to the output when the input state is in the network's sub-space, it is possible to reduce the size of the input
...
a decomposition that relies on the optimal policy in the previous subsection, I will now discuss the effect of decomposing in a way
that reduces the number of possible active input elements. I define a possible active input element as an element that can be both
a 0 or a 1, given the state-decomposition. For example, given the hybrid encoding with 625 elements corresponding to the positions
in the grid and 1 element corresponding to the destination, decomposing by position gives 312 (or 313) active position elements
and 1 active destination element, while decomposing by position leaves 625 position elements while making the 1 destination
element inactive. Thus, decomposing by position greatly reduces the number of active elements, while decomposing by destination
only changes the status of 1 element. This might justify the reduced performance of the decomposition by position. A further
experiment was run to validate this hypothesis, using standard one-hot encoding instead of the hybrid encoding. This is because
with standard on-hot-encoding, both types of decomposition roughly half the number of active neurons. 
...
decomposing by position. This can be attributed to the change only involving the "destination" subsection of the input state. Thus,
when the decomposition is by position, the change in performance going from hybrid to one-hot encoding is equivalent to the one
that occurs for DQN, which also gets slightly worse. Conversely, going from hybrid to one-hot allows the decomposition to have the
previously described effect of making some of the inputs inactive.
...
\end{equation}
where $\pi$ is the policy at the time of the update. Figure-\ref{fig:SARSA-progressive} shows that using expected SARSA seems to improve the
initial performance, as rewards never fall below -200. 
...
While the transfer learning techniques in the previous subsection show superior performance to DQN, this is an unfair comparison, as
they rely on the previously trained networks 'NN0' and 'NN1'. However, the two combining techniques can be modified to work in one
stage, thus simply providing a decomposition rather than doing transfer learning. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/simultaneous.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/modified-progressive.pdf}
