...
the experienced samples (time-steps) are used to learn a policy. The sample efficiency affects how quickly the algorithm can learn
...
in the the rewards are higher during the training phase, while they are similar upon convergence when compared to standard on-hot
...
One of the factors influencing the speed of learning of deep reinforcement learning algorithm is the size of the neural network.
...
The original state decomposition architecture proposed in the previous chapter 
Say that get stuck at -200 Say that two stage method has bad initialisation of second stage. Show figure of the bad initialisation
Say that for this reason try one stage initialisation
