...
sub-space of the input space, thus the name 'DeltaSwitch'. The updats of the network are carried out by treating the whole network
as a single unit. $\delta$ is chosen to be small, so that networks others than the one corresponding to the input state only have
a small impact on the output values. As a results, each sample used in an update of the network causes significant change in the
...
for through the use of a combininig NN.
...
and that doing so using another neurla network might be complicated and lead to stability issues and lack of convergence.
