...
This is the method of state-decomposition that this report focuses on, which is based on ignoring state transitions with
probability lower than the threshold $\tau$. There are different ways to decompose the state-space, the choice of which might
depends on the specific problem that the method is applied to. This is discussed further in
section-\ref{sec:different-decompositions}.
...
agent. In the first stage of training, a separate DQN agent is trained for each of these sub-spaces. To do so, we consider for
each sub-space's agent only those state transitions that start and end within the corresponding sub-space. Once the sub-spaces'
agents converge, these should be combined to form a global agent that also takes into account transitions between the different
sub-spaces. This is done by feeding the action values of the sub-spaces' agents, which are the outputs of their neural networks,
...
smaller MDPs than on one single MDP having as the number of states the sum of the number of states of the smaller MDPs. By then
...
networks of the sub-spaces' agents after the first stage are not far from the values they take once the global agent converges in
...
that use a function approximator to evaluate the action-values given in input the states. This method however wouldn't be as
...
algorithm. In this case, applying the state-decomposition would simply ignore the transitions between different sub-spaces without
...
sub-spaces are made unlikely by the characteristics of the environemnt itself, such as moving between two places separated by a
...
The decomposition into sub-spaces of states given the state-transition table can be done using Algorithm-\ref{algo:decompose},
which takes in input the state transition matrix. The states are initialised as all separate sub-spaces. Then we iterate through
all transition probabilities and given that the transition probability from state $i$ to state $j$ is higher than $\tau$ we join
the sub-spaces containing the two states. After iterating through all transitions this returns the decomposed sub-spaces.
