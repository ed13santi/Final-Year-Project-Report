...
the first stage of training, a separate DQN agent is trained for each of these sub-spaces. To do so, we consider for each
sub-space's agent only those state transitions that start and end within the corresponding sub-space. Once the sub-spaces' agents
...
\section[Decomposing the state-strans. matrix]{Decomposing the state-transition matrix} \label{sec:decomposing-trans-matrix}
